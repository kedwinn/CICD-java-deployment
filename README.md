...
Everytime developers, make releases, modify the code or push the code to github, jenkins would fetch the code from the designated Github repo by webhook association, and with the use of maven integration build cycle, would validate, compile, test, verify, install and package the code into a war file. Slack would help create that continuous feedback loop via intergration with Jenkins, Prometheus Alert manager by providing notifications in the event of any failures. The Archifact would be then deployed and pushed to sonarqube and checkstyle for code analysis done via unit, intergration, quality and format testing. if successful, it is then pushed down to Nexus which serves as the arficat repo for your monolithic archifacts, Maven dependencies and docker images.Ansible would pull the archifact from the Nexus Repo and push to the  Dev and Stage-Env for UAT testing and if approved  to the Prod-Env. Prometheus would be used as the continuous monitoring tool serving as tagret to the differnt environments via the use of alert manager and Grafana the data visualisation tool which helps view the different application, infrastructure, environment or pipeline metrics.
